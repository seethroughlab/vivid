...
void ChainVisualizer::render(const FrameInput& input, vivid::Context& ctx) {
    ...
    for (size_t i = 0; i < operators.size(); ++i) {
        ...
        if (kind == vivid::OutputKind::Texture) {
            ...
            // Fallback: inline visualizations for operators that haven't migrated yet
            ...
            } else if (auto* synth = dynamic_cast<audio::Synth*>(audioOp)) {
                // Synth - ADSR envelope + waveform icon
                dl->AddRectFilled(min, max, IM_COL32(25, 35, 55, 255), 4.0f);

                float env = synth->envelopeValue();
                audio::Waveform wf = synth->waveform();
                ...
                if (wf == audio::Waveform::Sine) {
                    for (int i = 0; i < 16; i++) {
                        float t1 = i / 16.0f;
                        float t2 = (i + 1) / 16.0f;
                        float y1 = iconY + iconSize * 0.5f - std::sin(t1 * 6.28f) * iconSize * 0.4f;
                        float y2 = iconY + iconSize * 0.5f - std::sin(t2 * 6.28f) * iconSize * 0.4f;
                        dl->AddLine(ImVec2(iconX + t1 * iconSize, y1),
                                   ImVec2(iconX + t2 * iconSize, y2), waveColor, 1.5f);
                    }
                } else if (wf == audio::Waveform::Square || wf == audio::Waveform::Pulse) {
                ...
                } else if (wf == audio::Waveform::Saw) {
                    float midY = iconY + iconSize * 0.5f;
                    float amp = iconSize * 0.35f;
                    dl->AddLine(ImVec2(iconX, midY + amp), ImVec2(iconX + iconSize * 0.5f, midY - amp), waveColor, 1.5f);
                    dl->AddLine(ImVec2(iconX + iconSize * 0.5f, midY - amp), ImVec2(iconX + iconSize * 0.5f, midY + amp), waveColor, 1.5f);
                    dl->AddLine(ImVec2(iconX + iconSize * 0.5f, midY + amp), ImVec2(iconX + iconSize, midY - amp), waveColor, 1.5f);
                } else if (wf == audio::Waveform::Triangle) {
                    float midY = iconY + iconSize * 0.5f;
                    float amp = iconSize * 0.35f;
                ...
                float a = static_cast<float>(synth->attack);
                float d = static_cast<float>(synth->decay);
                float s = static_cast<float>(synth->sustain);
                float r = static_cast<float>(synth->release);
            ...
            } else if (auto* polySynth = dynamic_cast<audio::PolySynth*>(audioOp)) {
                // PolySynth - voice activity + waveform
                dl->AddRectFilled(min, max, IM_COL32(30, 25, 50, 255), 4.0f);

                int activeVoices = polySynth->activeVoiceCount();
                int maxVoices = static_cast<int>(polySynth->maxVoices);
                float maxEnv = polySynth->maxEnvelopeValue();
                audio::Waveform wf = polySynth->getWaveform();
                ...
                for (int i = 0; i < maxVoices && i < 8; i++) {  // Show max 8 dots
                    float dotY = dotsStartY + i * dotSpacing;
                    ImU32 dotColor = (i < activeVoices)
            ...
            } else if (auto* fmSynth = dynamic_cast<audio::FMSynth*>(audioOp)) {
                // FMSynth - 4-operator algorithm diagram
                dl->AddRectFilled(min, max, IM_COL32(35, 25, 45, 255), 4.0f);

                audio::FMAlgorithm algo = fmSynth->algorithm();

                // Get operator envelope values for brightness
                float opEnv[4];
                for (int i = 0; i < 4; i++) {
                    opEnv[i] = fmSynth->operatorEnvelope(i);
                }

                // Operator positions (2x2 grid layout)
                // Op1=top-left, Op2=top-right, Op3=bottom-left, Op4=bottom-right
                ...
                // Define algorithm connections: {from, to} pairs, -1 means carrier (output)
                ...
                switch (algo) {
                    case audio::FMAlgorithm::Stack4:
                        // 1→2→3→4 (4 is carrier)
                        connections = {{0, 1}, {1, 2}, {2, 3}};
                        carriers = {3};
                        break;
                    case audio::FMAlgorithm::Stack3_1:
                        // 1→2→3, 4 (3 and 4 are carriers)
                        connections = {{0, 1}, {1, 2}};
                        carriers = {2, 3};
                        break;
                    case audio::FMAlgorithm::Parallel:
                        // All carriers
                        carriers = {0, 1, 2, 3};
                        break;
                    case audio::FMAlgorithm::Pairs:
                        // 1→2, 3→4
                        connections = {{0, 1}, {2, 3}};
                        carriers = {1, 3};
                        break;
                    case audio::FMAlgorithm::Branch2:
                        // 1→2, 1→3, + 4
                        connections = {{0, 1}, {0, 2}};
                        carriers = {1, 2, 3};
                        break;
                    case audio::FMAlgorithm::Branch3:
                        // 1→2,3,4
                        connections = {{0, 1}, {0, 2}, {0, 3}};
                        carriers = {1, 2, 3};
                        break;
                    case audio::FMAlgorithm::Y:
                        // 1→2, 1→3, 2+3→4
                        connections = {{0, 1}, {0, 2}, {1, 3}, {2, 3}};
                        carriers = {3};
                        break;
                    case audio::FMAlgorithm::Diamond:
                        // 1→2, 1→3, 2→4, 3→4
                        connections = {{0, 1}, {0, 2}, {1, 3}, {2, 3}};
                        carriers = {3};
                        break;
            ...
            } else if (auto* compressor = dynamic_cast<audio::Compressor*>(audioOp)) {
                // Compressor - gain reduction meter
                dl->AddRectFilled(min, max, IM_COL32(40, 35, 30, 255), 4.0f);

                float grDb = compressor->getGainReduction();  // Negative dB value
                float grNorm = std::min(1.0f, std::abs(grDb) / 20.0f);  // Normalize to 0-1 for up to -20dB

                // Draw threshold line (horizontal)
                float threshNorm = (static_cast<float>(compressor->threshold) + 60.0f) / 60.0f;
            ...
            } else if (auto* limiter = dynamic_cast<audio::Limiter*>(audioOp)) {
                // Limiter - ceiling indicator + gain reduction
                dl->AddRectFilled(min, max, IM_COL32(45, 30, 30, 255), 4.0f);

                float grDb = limiter->getGainReduction();
                ...
                float openAmount = gateGain;  // 0 = closed, 1 = open
                int numBars = 5;
                float barSpacing = gateW / (numBars + 1);

                for (int i = 0; i < numBars; i++) {
                    float barX = gateX + barSpacing * (i + 1);
                    float barTop = gateY + 4 + (gateH - 8) * (1.0f - openAmount) * 0.5f;
                    float barBot = gateY + gateH - 4 - (gateH - 8) * (1.0f - openAmount) * 0.5f;

                    ImU32 barColor = isOpen
                        ? IM_COL32(100, 200, 120, 255)   // Green when open
                        : IM_COL32(150, 100, 100, 200);  // Red-ish when closed

                    dl->AddLine(ImVec2(barX, barTop), ImVec2(barX, barBot), barColor, 2.0f);
                }

                // Status text
                const char* status = isOpen ? "OPEN" : "GATE";
                ImVec2 textSize = ImGui::CalcTextSize(status);
                ImU32 textColor = isOpen ? IM_COL32(100, 255, 120, 255) : IM_COL32(180, 100, 100, 200);
                dl->AddText(ImVec2(cx - textSize.x * 0.5f, max.y - 16), textColor, status);

            } else if (auto* filter = dynamic_cast<audio::AudioFilter*>(audioOp)) {
                // AudioFilter - frequency response curve
                dl->AddRectFilled(min, max, IM_COL32(30, 35, 50, 255), 4.0f);

                audio::FilterType fType = filter->filterType();
                float cutoffHz = static_cast<float>(filter->cutoff);
                float q = static_cast<float>(filter->resonance);
                float gainDb = static_cast<float>(filter->gain);
                ...
                float cutoffNorm = std::log10(cutoffHz / 20.0f) / std::log10(1000.0f);  // 0 to 1 for 20Hz to 20kHz
                ...
                auto computeFilterResponse = [&](float freqNorm) -> float {
                    ...
                    float freq = 20.0f * std::pow(1000.0f, freqNorm);  // 20Hz to 20kHz
                    float ratio = freq / cutoffHz;

                    switch (fType) {
                        case audio::FilterType::Lowpass: {
                            // 12dB/octave rolloff approximation with resonance peak
                        ...
                        case audio::FilterType::Highpass: {
                        ...
                        case audio::FilterType::Bandpass: {
                            float bw = 1.0f / q;
                            float response = 1.0f / (1.0f + std::pow((ratio - 1.0f/ratio) / bw, 2.0f));
                            return response;
                        }
                        case audio::FilterType::Notch: {
                            float bw = 1.0f / q;
                            float response = 1.0f - 1.0f / (1.0f + std::pow((ratio - 1.0f/ratio) / bw, 2.0f));
                            return response;
                        }
                        case audio::FilterType::Lowshelf:
                        case audio::FilterType::Highshelf:
                        case audio::FilterType::Peak:
                            // Simplified: just show a boost/cut at cutoff
                            return 1.0f;
                        default:
                            return 1.0f;
                    }
                ...
                for (int i = 0; i < NUM_POINTS - 1; i++) {
                    ...
                    // Convert to Y position (response 0-1 maps to curve height)
                    float y1 = curveY + curveH * (1.0f - r1 * 0.9f);  // Leave some margin
                ...
                switch (fType) {
                    case audio::FilterType::Lowpass: typeLabel = "LP"; break;
                    case audio::FilterType::Highpass: typeLabel = "HP"; break;
                    case audio::FilterType::Bandpass: typeLabel = "BP"; break;
                    case audio::FilterType::Notch: typeLabel = "NOTCH"; break;
                    case audio::FilterType::Lowshelf: typeLabel = "LSHF"; break;
                    case audio::FilterType::Highshelf: typeLabel = "HSHF"; break;
                    case audio::FilterType::Peak: typeLabel = "PEAK"; break;
                }
