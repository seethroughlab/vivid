...
- The current execution model uses a structural split: static lane counts use `LaneLiftGroup` instance duplication, dynamic identity-bearing collections use manual operator loops with `vivid_lane_state()`. This split is intermediate, not final. The target is a runtime where execution strategy is a planner-selected backend, not a permanent structural category. See `docs/lanes-execution-strategy-alignment.md`.
