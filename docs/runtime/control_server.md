...
| `sample_node_outputs` | `node_id`, `duration_seconds`, `interval_ms`, `include_lanes` | Time-series sampling of a node's output port values, including additive audio activity fields on audio-buffer outputs and per-sample node audio telemetry. Sampling is incremental across runtime ticks so it observes frame/audio progress without sleeping on the main loop. |
