...
**Status:** Phases 0–3 + Phase 4a/4b/4c of the [lane-value clean-break](../plans/lane-value-model-clean-break.md)
are **done** (4a float, 4b many-string, 4c GPU/texture): `VividFrameContext`/`VividGpuContext`
`values`/`value_outputs` cover float + STRING_LANES + texture, backed by the existing lane/string-lane/texture
transport. The vocabulary (`value_model.h`) + value views (`value_view.h`) are wired into the
