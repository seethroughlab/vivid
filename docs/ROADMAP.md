...
Note: the operators built in these phases are infrastructure proofs, not product deliverables. Vivid's value is the environment — the hot-reload loop, the experimentation interfaces, the LLM integration — not a shipped operator library. In the age of LLM-assisted development, writing an operator that does exactly what you want is cheap. What adds value is making experimentation and discovery possible and productive. Most operators in a Vivid project are generated by the LLM during the session, authored for the specific creative intent, and kept in the project directory. The seed operators and phase test cases exist to validate the architecture and serve as examples the LLM learns from.
...
**Implementation notes:**
- `operators/control/state_machine/state_machine.cpp` — up to 8 named states with per-state duration
- Three transition modes: sequential (clock-driven advance), manual (trigger input), threshold (signal crosses threshold value)
- Three crossfade modes: cut (instant), crossfade (blend), morph — with configurable crossfade duration in bars
- Beat-quantized transitions via `quantize` parameter (transitions land on bar boundaries)
- Outputs: current state index, state progress (0–1), transition trigger, bar/beat counters, crossfade mix amount
- Per-state parameter presets stored in graph JSON (`StatePresetMapping` in `src/runtime/graph.h`)
- Loop mode for cycling through states; non-loop mode stops at final state

---

#### Interstitial: macOS Native Menu Bar

Between Phase 21 and Phase 22, macOS native menu bar support was added as cross-cutting infrastructure:

- `src/runtime/macos_menu.h` / `macos_menu.mm` — Objective-C++ NSMenu integration
- File menu: Open (Cmd+O via NSSavePanel), Save (Cmd+S), Export Standalone... (NSSavePanel)
- Preferences menu item (Cmd+,)
- `MenuCallbacks` struct for decoupled event handling from the main loop
- Native keyboard shortcuts via NSMenuItem key equivalents

---

## Tier 6: Production Readiness

### Phase 22: Export / Standalone Builds — DONE
...
**Implementation notes:**
- `src/export/export_pipeline.cpp` / `export_pipeline.h` — `ExportPipeline` class orchestrates the full export
- Static registry generation: each operator compiled with `-D` symbol renaming (`vivid_descriptor` → `vivid_descriptor_<type>`) to avoid link-time collisions, then a `static_registry.cpp` is generated with forward declarations and `register_builtin()` calls for all included operators
- Graph embedding: graph JSON embedded as a C++ raw string literal in a generated header, loaded via `Graph::load_from_string()`
- Tree-shaking: graph JSON parsed to extract referenced operator types; only those operators (plus any `--extra-operators`) are compiled and linked
- WGSL filter preset embedding: data-driven `.wgsl` files embedded as string literals for graph-referenced filters
- `src/export/standalone_main.cpp` — minimal main() for standalone builds (GLFW + GPU context + audio engine + scheduler, no UI/hot-reload/REPL)
- `src/export/standalone.cmake.in` — CMake template configured per-export with operator sources, Dawn path, and embedded files
- Operator manifest generated at CMake configure time (`build/operator_manifest.json`) mapping type names → source paths
- CLI: `vivid export --graph <path> --output <name> [--output-dir <path>] [--headless] [--control-server] [--extra-operators ...]`
- GUI: File > Export Standalone... triggers NSSavePanel → runs ExportPipeline from the main loop

### Phase 23: Operator Sharing Ecosystem

`vivid install github.com/user/library` → clone → compile from source. Library manifest. Template repo. Search path resolution (project-local → user global → libraries → seed). The typical lifecycle: the LLM generates an operator for a specific session, the user refines it, and if it's general enough, they publish it as a library for others to install. The ecosystem exists to make this sharing frictionless, not to replace LLM generation as the primary way operators come into existence.
