...
**Core Admission Policy:** The core ships operators that provide a compelling out-of-box experience for music and visual performance. An operator belongs in core if it (a) is a fundamental building block that most graphs depend on (e.g. LFO, Clock, Math, Gain), or (b) is essential for a day-1 music/visual demo (e.g. drums, samplers, sequencers). Packages are for specialized or opinionated functionality (3D rendering, physics, ML), large external dependencies (CEF browser), or stylistic effects (glitch, wavetable). Users can always clone/modify any core operator and publish their own prefixed packages.
...
### 2.9 Ship a Compelling Core

The Vivid runtime is small, but the core ships enough operators for a compelling day-1 experience — drums, samplers, sequencers, effects, and GPU visuals work out of the box. The core handles graph execution, domain bridging, and the operator plugin interface. Specialized or opinionated functionality lives in packages (3D, physics, glitch, wavetable, browser, ML). Most custom operators are generated by the LLM during a session, authored for the specific project, and kept in the project directory. The operator contract is the most important API surface in the system because every LLM-generated operator is written against it.
...
- **libcurl**: HTTP fetches for package catalog and appcast update metadata.
- **Midifile**: Standard MIDI file parsing via pinned FetchContent/static target.
- **efsw**: cross-platform file watching for hot reload.
- **TinyXML-2**: lightweight XML parsing for appcast update metadata.
...
4. **Core operators** — `vivid/operators/` — operators shipped with Vivid covering synthesis, effects, drums, samplers, sequencers, and GPU visuals
...
> **Resolved:** 40+ core control operators shipped including LFO, Clock, Math, Envelope, Gate, Random, Smooth, MIDI Input, OSC In/Out, Keyboard, Mouse, Logic, FFT Analysis, Stack, Alternate, StepCounter, Sequencer, DrumSequencer, Arpeggiator, Euclidean, PatTransform, ChordProgression, StateMachine, Tracker, and more. Core audio includes drums (Kick, Snare, HiHat, Clap, Cymbal, Tom), samplers (SP404, Sampler, Slicer, GranularSynth, Vocoder, SpectralFreeze), and a full effects chain.
