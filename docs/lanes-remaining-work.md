...
**Goal:** Validate that InstancePerLane and LoopBased produce identical results, and migrate enough operators to make the model real.

**Why first:** Everything else builds on confidence that the execution strategy alignment actually works. Right now the LoopBased path exists but has no runtime proof.
...
**Scope:** ~600 LOC. **Risk:** Medium — touches core audio operators.
...
**Goal:** Clean up naming residue and formalize the planner.
...
### F2. Formal runtime planner (if needed)
- Currently the "planner" is Pass 4c with hardcoded rules
- If more backends arrive (GPU compute), formalize as `LaneExecutionPlan`
- May not be needed if the heuristics stay simple
...
```
A (prove execution model)     validates the strategy alignment
 ↓
...
E (GPU compute)               validates backend independence
 ↓
F (polish + naming)           cleanup
```

A is the most important near-term work — without it, the LoopBased path is infrastructure without proof. B and C are independently valuable. D depends on B/C for some scenarios. E is the long-term target. F can happen anytime but is most efficient last.
