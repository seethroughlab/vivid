...
**Current: v17.** The runtime loads any operator in **[v11, v17]** — every change since v11 has
...
| **v16** | ADR-0046. An operator-role hint. One OPTIONAL export (`uint32_t vivid_operator_role(void)`) the host `dlsym`s — absent ⇒ `VIVID_OP_ROLE_DEFAULT`, so nothing breaks for a dylib that omits it. `VividOperatorDescriptor` gains `role` (`VividOperatorRole`) appended at the END, filled from a virtual (`declared_operator_role`) for compiled-in ops or the dylib's export via the loader, so the host never reads it out of an older dylib's shorter struct. Lets an operator classify itself as a composable primitive (source / transform / adapter / renderer / sink) or a bundled `RECIPE` that ranks below primitives in the add-node chooser and `list_operator_catalog`. |
