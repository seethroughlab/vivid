...
public:
     ...
     *
     * This is called by the video decoder to feed audio data.
     * If the buffer is full, samples may be dropped.
     */
    void pushSamples(const float* samples, uint32_t frameCount);

    /**
     * @brief Get the current playback position in seconds.
     * Can be used for A/V sync.
     */
...
private:
    ...
    // Track playback position for sync
