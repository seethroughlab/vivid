...
static_assert(sizeof(InstancedUniforms) == 304, "InstancedUniforms size check");
...
InstancedRender3D& InstancedRender3D::mesh(GeometryOperator* geom) {
    m_geometryOp = geom;
    m_mesh = nullptr;
    if (geom) {
        setInput(0, geom);
...
void InstancedRender3D::process(Context& ctx) {
    // Ensure pipeline and depth buffer are created
    createPipeline(ctx);
    ...
    if (m_geometryOp) {
        meshToRender = &m_geometryOp->outputMesh();
    }
