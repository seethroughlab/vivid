...
bool WebServer::drawVisualization(VizDrawList* dl, float minX, float minY, float maxX, float maxY) {
    float w = maxX - minX;
    float h = maxY - minY;
    ...
    // Background circle
    uint32_t bgColor = m_running ? VIZ_COL32(30, 60, 80, 255) : VIZ_COL32(60, 30, 30, 255);
    ...
    uint32_t iconColor = m_running ? VIZ_COL32(100, 200, 255, 255) : VIZ_COL32(180, 180, 180, 255);
    float iconR = r * 0.5f;

    // Horizontal lines
    dl->AddCircle(VizVec2(cx, cy), iconR, iconColor, 24, 1.5f);
    dl->AddLine(VizVec2(cx - iconR, cy), VizVec2(cx + iconR, cy), iconColor, 1.5f);
    dl->AddLine(VizVec2(cx - iconR * 0.85f, cy - iconR * 0.5f), VizVec2(cx + iconR * 0.85f, cy - iconR * 0.5f), iconColor, 1.0f);
    dl->AddLine(VizVec2(cx - iconR * 0.85f, cy + iconR * 0.5f), VizVec2(cx + iconR * 0.85f, cy + iconR * 0.5f), iconColor, 1.0f);

    // Vertical ellipse (simplified)
    dl->AddLine(VizVec2(cx, cy - iconR), VizVec2(cx, cy + iconR), iconColor, 1.5f);

    // Port number below
    char portStr[16];
    snprintf(portStr, sizeof(portStr), ":%d", m_port);
    VizTextSize portSize = dl->CalcTextSize(portStr);
    dl->AddText(VizVec2(cx - portSize.x * 0.5f, cy + r * 0.6f), VIZ_COL32(150, 150, 150, 255), portStr);
    ...
    if (clients > 0) {
        ...
        VizTextSize countSize = dl->CalcTextSize(countStr);
        dl->AddText(VizVec2(cx - countSize.x * 0.5f, maxY - countSize.y - 2), VIZ_COL32(100, 200, 255, 200), countStr);
    }
