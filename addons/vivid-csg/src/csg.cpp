...
CSGMesh Solid::toMesh() const {
    ...
    // Create vertices with normals and triplanar UVs
    for (size_t i = 0; i < mesh.NumVert(); ++i) {
        Vertex3D v;
        v.position = glm::vec3(
            mesh.vertProperties[i * numProp + 0],
        ...
        // Triplanar UV mapping based on position and normal
        // Choose UV projection based on dominant normal axis
        glm::vec3 absNormal = glm::abs(v.normal);

        if (absNormal.x >= absNormal.y && absNormal.x >= absNormal.z) {
            // X-facing: project onto YZ plane
            v.uv = glm::vec2(v.position.z, v.position.y);
            v.tangent = glm::vec3(0.0f, 0.0f, v.normal.x > 0 ? 1.0f : -1.0f);
            ...
            v.uv = glm::vec2(v.position.x, v.position.y);
            v.tangent = glm::vec3(v.normal.z > 0 ? 1.0f : -1.0f, 0.0f, 0.0f);
        }
