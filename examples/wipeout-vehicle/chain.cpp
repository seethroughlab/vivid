...
static float cameraDistance = 14.0f;  // Increased for larger vehicle
...
Mesh3D buildFuselage(Context& ctx) {
    ...
    // Fuselage dimensions - much longer and narrower needle shape
    float length = 9.0f;    // Much longer
    float width = 0.45f;    // Narrower
    float height = 0.25f;   // Lower profile
    float spineHeight = 0.08f;
    int segments = 10;

    // Cross-section profile at each segment
    // Format: {x-position, width-scale, height-scale, spine-scale}
    // Long needle taper from front, wider at rear where hulls attach
    float profile[][4] = {
        {-0.50f, 0.00f, 0.30f, 0.0f},   // Sharp nose tip
        {-0.42f, 0.12f, 0.45f, 0.1f},   // Nose taper
        {-0.32f, 0.28f, 0.60f, 0.3f},   // Front needle
        {-0.20f, 0.45f, 0.75f, 0.5f},   // Mid-front
        {-0.05f, 0.65f, 0.88f, 0.7f},   // Mid
        { 0.10f, 0.85f, 1.00f, 0.9f},   // Widening toward rear
        { 0.25f, 1.00f, 1.00f, 1.0f},   // Max width (hull attachment)
        { 0.35f, 0.95f, 0.95f, 0.9f},   // Rear body
        { 0.42f, 0.75f, 0.80f, 0.6f},   // Rear taper
        { 0.50f, 0.40f, 0.55f, 0.3f},   // Rear end
    };
...
Mesh3D buildSidePod(Context& ctx, float side) {
    ...
    float podLength = 3.0f;    // Longer to match sleeker fuselage
    float podWidth = 0.5f;
...
Mesh3D buildRearWing(Context& ctx) {
    ...
    float span = 2.4f;     // Span to match pod width
    float chord = 0.5f;
...
void regenerateLivery(Context& ctx) {
    ...
    // Use custom fonts for team numbers and text
    gen.setNumberFont("examples/wipeout-vehicle/fonts/Game Of Squids.ttf");
    gen.setTextFont("examples/wipeout-vehicle/fonts/SilverStream.ttf");
...
void update(Chain& chain, Context& ctx) {
    ...
    if (scroll != 0) {
        cameraDistance = glm::clamp(cameraDistance - scroll * 0.5f, 6.0f, 35.0f);
        updateCamera();
    }
    ...
    // Toggle vertex-lit rendering mode (V key)
    ...
    if (useVertexLit) {
        ...
        VertexLitMaterial vlBodyMat = VertexLitMaterial::ps1(palette.primary);
        vlBodyMat.diffuseMap = useLivery ? &liveryTexture : nullptr;
        if (useLivery) vlBodyMat.diffuse = glm::vec3(1.0f);

        VertexLitMaterial vlPodMat = VertexLitMaterial::ps1(palette.secondary);
        vlPodMat.diffuseMap = useLivery ? &liveryTexture : nullptr;
        if (useLivery) vlPodMat.diffuse = glm::vec3(1.0f);

        VertexLitMaterial vlCockpitMat = VertexLitMaterial::flat(glm::vec3(0.1f, 0.1f, 0.15f));
        vlCockpitMat.quantizeSteps = 2;

        VertexLitMaterial vlEngineMat = VertexLitMaterial::flat(glm::vec3(0.9f, 0.9f, 0.92f));
        vlEngineMat.emissive = glm::vec3(1.0f, 0.5f, 0.2f) * (0.5f + engineGlow);
        vlEngineMat.quantizeSteps = 3;

        VertexLitMaterial vlFinMat = VertexLitMaterial::ps1(palette.accent);
        vlFinMat.diffuseMap = useLivery ? &liveryTexture : nullptr;

        VertexLitMaterial vlWingMat = vlFinMat;

        VertexLitMaterial vlCanardMat = VertexLitMaterial::ps1(palette.primary * 0.6f);
