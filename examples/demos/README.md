...
| **constellation** | **polyphonic notes → geometry** — one glowing instance per LIVE note | a Surge pad plays A-minor chords + an arp | the **NoteInstancer** op draws an instance per held note (pitch → x + colour, velocity → size); chords bloom into pitch-coloured clusters, the arp sweeps a trail → **Feedback** |
