...
def build(v: Vivid, save: bool = True):
    v.reset()
    # ADR-0054: InstanceGrid is no longer a core operator — single-look ops moved out into installable
    # example packages, and a demo that uses one CARRIES it. The project folder vendors the source
    # (see projects/grid/vivid-package.json), so installing it here registers it live before the nodes
    # below ask for it. Without this the build dies on `unknown op 'InstanceGrid'`.
    v.call("install_operator_package", path=PROJECT)
    v.bpm(BPM)
