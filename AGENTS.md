...
When generating operators, write clean C++ against the current `operator_api` headers. `operator_codegen` generates the `extern "C"` boundary at build time — no registration macro is needed. Do not add binary-compatibility scaffolding (struct_size fields, reserved slots, interface tables).
...
- The dlopen boundary uses `extern "C"` functions generated by `operator_codegen` at build time
...
- For checked-in live investigations, prefer `./.venv-mcp/bin/python scripts/mcp_bridge_smoke.py ...` over ad hoc control-server `curl` requests so the bridge path itself is exercised.
