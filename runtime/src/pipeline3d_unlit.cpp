...
const char* UNLIT = R"(
// ============================================================================
// Unlit Shader - Direct color output with no lighting
// ============================================================================
...
// ============================================================================
// Pipeline3DUnlit Implementation
// ============================================================================

Pipeline3DUnlit::~Pipeline3DUnlit() {
    destroy();
}

void Pipeline3DUnlit::destroy() {
    destroyDepthBuffer();
...
void Pipeline3DUnlit::ensureDepthBuffer(int width, int height) {
...
void Pipeline3DUnlit::destroyDepthBuffer() {
...
bool Pipeline3DUnlit::init(Renderer& renderer) {
    destroy();
    renderer_ = &renderer;

    return createPipeline(shaders3d::UNLIT);
}

bool Pipeline3DUnlit::createPipeline(const std::string& shaderSource) {
    ...
    if (!shaderModule_) {
        std::cerr << "[Pipeline3DUnlit] Failed to create shader module\n";
        return false;
    }
    ...
    // Blend state (with alpha blending for opacity)
    ...
    if (!pipeline_) {
        std::cerr << "[Pipeline3DUnlit] Failed to create render pipeline\n";
        return false;
    }
    ...
    bufferDesc.size = 32;   // Material (color + opacity + hasTexture + padding)
    ...
    std::cout << "[Pipeline3DUnlit] Created successfully\n";
    return true;
}

void Pipeline3DUnlit::render(const Mesh3D& mesh, const Camera3D& camera, const glm::mat4& transform,
    ...
    UnlitMaterialUniform materialData = makeUnlitMaterialUniform(material);
    ...
    // Material bind group (with texture)
    {
        ...
        entries[0].size = sizeof(UnlitMaterialUniform);

        // Texture view
        WGPUTextureView texView = nullptr;
        if (material.colorMap && material.colorMap->valid()) {
            TextureData* texData = getTextureData(*material.colorMap);
    ...
    if (!outputView) {
        std::cerr << "[Pipeline3DUnlit] Invalid output texture\n";
        return;
    }
