...
void PreviewServer::start() {
    ...
    if (!res.first) {
        std::cerr << "[PreviewServer] Failed to start: " << res.second << std::endl;
        return;
    }
    server_.start();
    running_ = true;
    std::cout << "[PreviewServer] Listening on port " << port_ << std::endl;
}
...
void PreviewServer::stop() {
    if (running_) {
        server_.stop();
        running_ = false;
        std::cout << "[PreviewServer] Stopped" << std::endl;
    }
}
...
void PreviewServer::onMessage(std::shared_ptr<ix::ConnectionState> state,
    ...
    if (msg->type == ix::WebSocketMessageType::Open) {
        std::cout << "[PreviewServer] Client connected from " << state->getRemoteIp() << std::endl;
    }
    else if (msg->type == ix::WebSocketMessageType::Close) {
        std::cout << "[PreviewServer] Client disconnected" << std::endl;
    }
        ...
        try {
            ...
            std::cerr << "[PreviewServer] Parse error: " << e.what() << std::endl;
        }
    }
    else if (msg->type == ix::WebSocketMessageType::Error) {
        std::cerr << "[PreviewServer] Error: " << msg->errorInfo.reason << std::endl;
    }
}
