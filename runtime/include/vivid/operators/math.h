...
/**
 * @brief Arithmetic operations on numeric values.
 *
 * Performs arithmetic operations on values from other operators
 * or constant values.
 *
 * Example:
 * @code
 * Math scale_;
 * scale_.a("lfo").b(100.0f).multiply();  // lfo.out * 100
 *
 * Math remap_;
 * remap_.a("input").remap().inMin(0).inMax(1).outMin(-1).outMax(1);
 * @endcode
 */
...
public:
    ...
    /// Set input A from another operator's output
    Math& a(const std::string& node) { inputA_ = node; useConstA_ = false; return *this; }
    /// Set input B from another operator's output
    Math& b(const std::string& node) { inputB_ = node; useConstB_ = false; return *this; }
    /// Set input A to a constant value
    Math& a(float val) { constA_ = val; useConstA_ = true; return *this; }
    /// Set input B to a constant value
    ...
    void process(Context& ctx) override {
        ...
        switch (operation) {
            case MathOp::Add:      result = valA + valB; break;
            case MathOp::Subtract: result = valA - valB; break;
            case MathOp::Multiply: result = valA * valB; break;
            case MathOp::Divide:   result = (valB != 0.0f) ? valA / valB : 0.0f; break;
            case MathOp::Min:      result = std::min(valA, valB); break;
            case MathOp::Max:      result = std::max(valA, valB); break;
            case MathOp::Clamp:    result = std::clamp(valA, valB, constB_); break;
            case MathOp::Remap: {
                float t = (valA - inMin_) / (inMax_ - inMin_ + 0.0001f);
                result = outMin_ + t * (outMax_ - outMin_);
                break;
            }
            case MathOp::Abs:   result = std::abs(valA); break;
            case MathOp::Sin:   result = std::sin(valA); break;
            case MathOp::Cos:   result = std::cos(valA); break;
            case MathOp::Pow:   result = std::pow(valA, valB); break;
            case MathOp::Sqrt:  result = std::sqrt(std::max(0.0f, valA)); break;
            case MathOp::Floor: result = std::floor(valA); break;
            case MathOp::Ceil:  result = std::ceil(valA); break;
            case MathOp::Fract: result = valA - std::floor(valA); break;
            case MathOp::Mix:   result = valA + (valB - valA) * constB_; break;
        }
