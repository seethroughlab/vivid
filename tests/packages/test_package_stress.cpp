...
static float node_scalar_output(const vivid::RuntimeCore& runtime, const std::string& node_id) {
    const auto* node = runtime.compiled_graph()->find_node(node_id);
    if (!node || node->output_values.empty()) return std::nanf("");
    return node->output_values[0];
}

static void check_finite_output(const vivid::RuntimeCore& runtime,
                                const std::string& node_id,
                                const char* msg) {
    const float value = node_scalar_output(runtime, node_id);
...
int main(int argc, char* argv[]) {
    ...
    check_finite_output(runtime, "pkg_live", "initial linked package output is finite");
    for (int cycle = 0; cycle < 4; ++cycle) {
        std::fprintf(stderr, "\n--- package cycle %d ---\n", cycle + 1);

        runtime.tick(0.0, 0.016, static_cast<uint64_t>(cycle));
        check_finite_output(runtime, "pkg_live", "linked package output stays finite");
        ...
        runtime.tick(0.0, 0.016, static_cast<uint64_t>(cycle + 10));
        {
            const auto* rebuilt_node = runtime.compiled_graph()->find_node("pkg_live");
        ...
        check_finite_output(runtime, "pkg_live", "rebuilt package output stays finite");
        ...
        const auto* missing_node = runtime.compiled_graph()->find_node("pkg_live");
        ...
        runtime.tick(0.0, 0.016, static_cast<uint64_t>(cycle + 20));
        const auto* recovered_node = runtime.compiled_graph()->find_node("pkg_live");
        ...
        check_finite_output(runtime, "pkg_live", "relinked package output stays finite");
    }
