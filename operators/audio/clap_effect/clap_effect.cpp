...
struct CLAPEffect : vivid::OperatorBase, vivid::AudioProcessable {
    ...
    void build_macro_events() {
        bank_.emit_changes([&](clap_id id, float v, const auto& entry) {
            double scaled = entry.min_val +
                static_cast<double>(v) * (entry.max_val - entry.min_val);
    ...
    void draw_inspector(VividInspectorContext* ctx) override {
        ...
        for (int i = 0; i < kMaxMacros; ++i) {
            ...
            // Native value (CLAP: linear rescale; no units)
...
VIVID_INSPECTOR_FULL_MODE(CLAPEffect)
