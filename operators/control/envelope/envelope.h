 ...
 * @tip To retrigger rhythmically, set clock_mode=metronome (default) or connect beat_phase from a Clock for a custom tempo.
...
struct Envelope : vivid::OperatorBase {
    ...
    void collect_ports(std::vector<VividPortDescriptor>& out) override {
        out.push_back({.name="gate",     .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="gate",    .semantic_shape="lane_array", .semantic_intent="per_note_gate", .description="Gate input for ADSR triggering. Accepts a scalar gate or per-voice lanes such as NoteBreakout/voice_gates.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="lane_ids", .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="lane_id", .semantic_shape="lane_array", .description="Per-voice lane IDs for stable envelope state across reallocation.", .multiplicity=VIVID_MULTIPLICITY_MANY});
    ...
    void process_frame(const VividFrameContext* ctx) {
        ...
        float phase_in = vivid::resolve_clock_phase(
            clock_mode.int_value(), ctx->input_values[2], vivid::metronome_transport(ctx));
