...
struct ShapeField : vivid::OperatorBase, vivid::GpuProcessable {
    ...
    void collect_ports(std::vector<VividPortDescriptor>& out) override {
        out.push_back({.name="pos_x",      .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="position_xy", .semantic_shape="lane_array", .semantic_intent="x_component", .description="Per-instance x position in [0, 1]. When connected, overrides layout.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="pos_y",      .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="position_xy", .semantic_shape="lane_array", .semantic_intent="y_component", .description="Per-instance y position in [0, 1]. When connected, overrides layout.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="size",       .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="amplitude_linear", .semantic_shape="lane_array", .semantic_intent="scale_multiplier", .description="Per-instance size multiplier on base_size. 1.0 = default.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="hue",        .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="color_hue", .semantic_shape="lane_array", .semantic_intent="hue_cycles", .description="Per-instance hue in [0, 1]. HSV(hue, 1, 1) replaces color_r/g/b when connected.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="brightness", .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="amplitude_linear", .semantic_shape="lane_array", .semantic_intent="per_note_amplitude", .description="Per-instance intensity in [0, 1]. Multiplies RGB and alpha.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="rotation",   .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="rotation_radians", .semantic_shape="lane_array", .semantic_intent="angle_turns", .description="Per-instance rotation in turns [0, 1]. Overrides the rotation LFO when connected.", .multiplicity=VIVID_MULTIPLICITY_MANY});
        out.push_back({.name="shape_idx",  .type=VIVID_PORT_SCALAR, .direction=VIVID_PORT_INPUT, .transport=VIVID_PORT_TRANSPORT_LANE_ARRAY, .semantic_tag="enum_index", .semantic_shape="lane_array", .semantic_intent="shape_selector", .description="Per-instance shape index: 0=Circle, 1=Triangle, 2=Square, 3=Pentagon, 4=Hexagon, 5=Star. Overrides the global shape param when connected; clamped to [0, 5].", .multiplicity=VIVID_MULTIPLICITY_MANY});
