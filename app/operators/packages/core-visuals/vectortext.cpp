...
@vertex fn vs_fan(@location(0) p: vec2f) -> @builtin(position) vec4f {
    // FreeType glyph outlines are Y-up and so is clip space, so pass glyph Y straight through —
    // the pipeline no longer inverts Y per pass (see fullscreenTriangle in gpu_common.h).
