// Core visual package operator: Type — a live TYPOGRAPHIC readout of a signal. Consumes a VividSignal
// and renders the currently-active elements as filled vector text (FreeType glyph outlines → stencil
// even-odd fill, exactly like VectorText): note names ("C4 E4 G4") or pitch numbers, rebuilt each time
// the set changes. The clean / Swiss / data-viz face of the reactivity toolkit — the picture literally
// spells out what's playing. Agnostic to the source (never refers to "notes" in its ports); a Notes
// signal drives it today. Reuses the VectorText fill pipeline; the string comes from the edge, not a file.
#include "operator_api/operator.h"
#include "operator_api/gpu_operator.h"
#include "operator_api/gpu_common.h"
#include "operator_api/element_geom.h"   // VividSignal, input_signal
#include "gpu/freetype_font.h"           // vivid::FtFont / ft_string_contours (package links freetype)
...
@vertex fn vs_fan(@location(0) p: vec2f) -> @builtin(position) vec4f {
    var q = vec2f(p.x, p.y) * (u.size * 1.6);        // glyph space → NDC (upright feeding Output)
...
struct TypeOp : vivid::OperatorBase, vivid::GpuProcessable {
    ...
    vivid::Param<float> size{"size", 0.18f, 0.f, 1.f}, x{"x", 0.5f, 0.f, 1.f}, y{"y", 0.5f, 0.f, 1.f};
    ...
    vivid::Param<float> mode{"mode", 0.f, 0.f, 1.f};   // 0 note names · 1 pitch numbers
    ...
    // Build the readout string from the signal's active elements (low → high), space-joined.
    std::string build_text(const VividSignal* sig, int md) {
        if (!sig || !sig->active || sig->active_count == 0) return "";
        std::vector<int> pitches;
        for (uint32_t i = 0; i < sig->active_count; ++i)
            pitches.push_back(static_cast<int>(std::lround(sig->active[i].pos * 127.f)));
        std::sort(pitches.begin(), pitches.end());
        std::string s;
        for (size_t i = 0; i < pitches.size(); ++i) {
            if (i) s += " ";
            s += (md == 1) ? std::to_string(pitches[i]) : note_name(pitches[i]);
        }
        return s;
    }
        ...
        if (!text_.empty() && font_.ok) {
            ...
            for (const auto& cont : fc.contours) {
                ...
                for (size_t i = 0; i < n; ++i) {
                    ...
                    v.push_back({ 0.f, 0.f }); v.push_back(nz(a)); v.push_back(nz(bb));
                }
            }
        }
