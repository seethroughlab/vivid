...
// Sampler::render — no audio device needed, it's pure math over vectors.
#include "audio/sampler.h"
#include "test_helpers.h"
#include <vector>

using vivid::session::Sampler;

// Render one full loop's worth of frames starting at beat 0.
static void render_loop(const Sampler& s, std::vector<float>& L, std::vector<float>& R, uint32_t frames) {
...
int main() {
    ...
    auto make_const = [&](float v) {
        Sampler s; s.sr = SR; s.loop_beats = 4.0; s.L.assign(SR, v); return s;   // 1s mono
    };

    // gain scales the output uniformly.
    {
        Sampler s = make_const(0.5f); s.gain = 1.0f;
    ...
    {
        Sampler s; s.sr = SR; s.loop_beats = 4.0;
