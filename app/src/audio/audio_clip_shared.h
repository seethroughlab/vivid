...
// Estimate BPM from the median inter-onset interval of the detected transients, folded
// by octaves into [bpm_lo, bpm_hi]. 0 if there aren't enough transients.
inline double estimate_bpm(const std::vector<TransientPoint>& tr, uint32_t sample_rate,
                           double bpm_lo = 70.0, double bpm_hi = 180.0) {
    if (tr.size() < 2 || sample_rate == 0) return 0.0;
    std::vector<double> gaps;
    for (size_t i = 1; i < tr.size(); ++i) {
        const double g = static_cast<double>(tr[i].source_sample - tr[i - 1].source_sample) / sample_rate;
        if (g > 1e-3) gaps.push_back(g);
    }
    if (gaps.empty()) return 0.0;
    std::sort(gaps.begin(), gaps.end());
    double bpm = 60.0 / gaps[gaps.size() / 2];   // 60 / median inter-onset interval (seconds)
    if (bpm <= 0.0) return 0.0;
    while (bpm < bpm_lo) bpm *= 2.0;
    while (bpm > bpm_hi) bpm *= 0.5;
    return bpm;
}

// Auto-warp: place a warp marker at each transient, snapped to the nearest beat implied
// by `bpm_hint` (0 = estimate it from the transients). Always anchors a marker at sample 0.
inline std::vector<WarpPoint> auto_warp(const std::vector<TransientPoint>& tr,
                                        uint32_t /*frame_count*/, uint32_t sample_rate,
                                        double bpm_hint) {
    const double bpm = bpm_hint > 0.0 ? bpm_hint : estimate_bpm(tr, sample_rate);
    std::vector<WarpPoint> out;
    if (bpm <= 0.0 || sample_rate == 0) return out;
    for (const auto& t : tr) {
        const double sec  = static_cast<double>(t.source_sample) / sample_rate;
        const double beat = std::round(sec * bpm / 60.0);
        out.push_back({t.source_sample, beat});
    }
    if (out.empty() || out.front().source_sample > 0) out.insert(out.begin(), {0u, 0.0});
    return sanitize_warp_points(std::move(out));
}
