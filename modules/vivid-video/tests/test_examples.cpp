// vivid-video Example Smoke Tests
// Verifies all vivid-video examples run without crashing using snapshot mode.
...
static fs::path getTempOutputPath(const std::string& testName) {
    ...
    return tempDir / ("vivid_video_example_" + testName + ".png");
}
...
TEST_CASE("vivid-video examples run without crash", "[smoke][examples][video]") {
    ...
    DYNAMIC_SECTION("Example: " << example) {
        ...
        fs::path examplePath = fs::path(getSourceDir()) / "modules" / "vivid-video" / "examples" / example;
