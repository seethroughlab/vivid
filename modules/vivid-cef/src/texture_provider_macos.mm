 ...
 * in standard CEF builds. This is a Chromium/CEF limitation -
 * OnAcceleratedPaint only works on Windows with D3D11 shared textures.
 *
...
public:
  ~TextureProviderMacOS() override { cleanup(); }
  ...
  void importFromCPU(const void *buffer, int width, int height) override {
    ...
    // CEF provides BGRA data, we store as BGRA (no swizzle needed for
    // BGRA8Unorm)
...
private:
  bool createTextures() {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      if (!m_textureViews[i]) {
        fprintf(stderr,
                "[TextureProviderMacOS] Failed to create texture view %d\n", i);
        return false;
      }
    }
  ...
  void importFromIOSurface(IOSurfaceRef surface) {
    ...
    bool isBGRA =
        (pixelFormat == kCVPixelFormatType_32BGRA || pixelFormat == 'BGRA');
    ...
    if (!isBGRA) {
      ...
      if (lastFormat != pixelFormat) {
        fprintf(stderr,
                "[TextureProviderMacOS] Unexpected IOSurface format: "
                "'%c%c%c%c' (0x%08x)\n",
                (char)((pixelFormat >> 24) & 0xFF),
                (char)((pixelFormat >> 16) & 0xFF),
                (char)((pixelFormat >> 8) & 0xFF), (char)(pixelFormat & 0xFF),
                pixelFormat);
    ...
    // kIOSurfaceLockReadOnly ensures we don't block CEF from writing to the
    // next surface
    kern_return_t lockResult =
    ...
    if (baseAddress) {
      ...
      WGPUExtent3D extent = {static_cast<uint32_t>(surfaceWidth),
                             static_cast<uint32_t>(surfaceHeight), 1};
      ...
      wgpuQueueWriteTexture(m_queue, &dst, baseAddress, dataSize, &layout,
                            &extent);
    }
