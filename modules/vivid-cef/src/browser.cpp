...
namespace {
    ...
    std::string resolveFileUrl(const std::string& url, const std::string& projectDir) {
...
void Browser::setInputEnabled(bool enabled) {
...
void Browser::processInput(Context& ctx) {
    if (!m_inputEnabled || !m_impl || !m_impl->isCreated()) return;
    ...
    // Mouse move
    if (bx != static_cast<int>(m_prevMouseX) || by != static_cast<int>(m_prevMouseY)) {
...
void Browser::processRawInput(const RawInputState& input) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (input.mouseButtons[i] != m_prevMouseButtons[i]) {
            m_impl->sendMouseButton(static_cast<MouseButton>(i), input.mouseButtons[i], bx, by, 1);
...
void Browser::sendMouseButton(MouseButton btn, bool pressed, int x, int y, int clicks) {
    if (m_impl && m_impl->isCreated()) {
        m_impl->sendMouseButton(btn, pressed, x - m_inputOffsetX, y - m_inputOffsetY, clicks);
    }
}

void Browser::sendMouseWheel(int x, int y, float deltaX, float deltaY) {
    if (m_impl && m_impl->isCreated()) {
        m_impl->sendMouseWheel(x - m_inputOffsetX, y - m_inputOffsetY, deltaX, deltaY);
    }
}
...
bool Browser::isLoading() const {
    return m_impl && m_impl->isLoading();
}
...
int Browser::browserWidth() const {
    return m_configWidth;
}

int Browser::browserHeight() const {
...
void Browser::init(Context& ctx) {
    ...
    if (!isCefInitialized()) {
        fprintf(stderr, "[Browser] CEF not initialized. Call initializeCef() first.\n");
    ...
    if (!m_impl->create(ctx.device(), m_configWidth, m_configHeight, m_transparent)) {
    ...
    // Set callbacks
    if (m_loadEndCallback) m_impl->setLoadEndCallback(m_loadEndCallback);
    if (m_consoleCallback) m_impl->setConsoleCallback(m_consoleCallback);
    if (m_cursorChangeCallback) m_impl->setCursorChangeCallback(m_cursorChangeCallback);
...
std::vector<ParamDecl> Browser::params() {
    return {
        ...
        {"frameRate", ParamType::Int, 1, 120, {static_cast<float>(m_frameRate), 0, 0, 0}}
    };
}
...
bool Browser::getParam(const std::string& name, float out[4]) {
    ...
    if (name == "frameRate") { out[0] = static_cast<float>(m_frameRate); return true; }
    return false;
}
...
bool Browser::setParam(const std::string& name, const float value[4]) {
    ...
    if (name == "frameRate") { setFrameRate(static_cast<int>(value[0])); return true; }
    return false;
}
...
void Browser::createFallbackTexture(Context& ctx) {
    ...
    wgpuQueueWriteTexture(ctx.queue(), &dst, pixels.data(), pixels.size(), &layout, &extent);
