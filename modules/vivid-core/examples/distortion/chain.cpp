...
//
// Shows various spatial distortion and edge detection effects with grid overlay
...
void setup(Context& ctx) {
    ...
    displace.source("base");
    displace.map("dispMap");
    displace.strength = 0.08f;
    displace.strengthX = 1.0f;
    displace.strengthY = 1.0f;

    // Output canvas for 2x2 grid layout
...
void update(Context& ctx) {
    ...
    chroma.amount = 0.01f + std::sin(t * 0.4f) * 0.015f;
    chroma.angle = t * 0.5f;

    // Animate barrel distortion
    auto& barrel = chain.get<BarrelDistortion>("barrel");
    barrel.curvature = 0.1f + std::sin(t * 0.3f) * 0.08f;

    // Animate edge detection
    auto& edge = chain.get<Edge>("edge");
    edge.strength = 1.5f + std::sin(t * 0.5f) * 1.0f;
    edge.threshold = 0.1f + std::sin(t * 0.2f) * 0.08f;

    // Animate displacement
    auto& displace = chain.get<Displace>("displace");
    displace.strength = 0.05f + std::sin(t * 0.35f) * 0.04f;
    displace.strengthX = 1.0f + std::sin(t * 0.6f) * 0.5f;
    displace.strengthY = 1.0f + std::cos(t * 0.6f) * 0.5f;
