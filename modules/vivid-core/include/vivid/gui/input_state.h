...
/**
 * @file input_state.h
 * @brief GUI-owned input state types (independent of vivid-core)
 *
 * This file defines the input state types used by the GUI module,
 * making it independent of vivid-core's FrameInput. This allows
 * the GUI module to be used without pulling in other vivid dependencies.
 *
 * The vivid application layer provides a conversion from FrameInput to InputState.
 */
...
/**
 * @brief Key codes for common keys (matches GLFW values)
 */
enum class Key : int {
    Space = 32,
    // Number keys
...
/**
 * @brief Input state for GUI rendering
 *
 * Contains all input information needed by GUI panels.
 * Independent of vivid-core's FrameInput to allow GUI module reuse.
 */
struct InputState {
...
/**
 * @brief Convert FrameInput to InputState
 *
 * This helper function is provided so that application code can easily
 * convert from vivid's FrameInput to the GUI module's InputState.
 *
 * Usage (in app layer):
 * @code
 ...
 *
 * // In your render loop:
 * gui::InputState guiInput = gui::toInputState(frameInput);
 * panelManager.render(canvas, guiInput, screenWidth, screenHeight, style);
 * @endcode
 */
// Note: The actual conversion function is implemented in the app layer
// to avoid including frame_input.h here. It can be a simple struct copy
// since the layouts match.
