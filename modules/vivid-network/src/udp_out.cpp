...
REGISTER_OPERATOR(UdpOut, "Network", "Send raw UDP packets", false);
...
bool UdpOut::drawVisualization(VizDrawList* dl, float minX, float minY, float maxX, float maxY) {
    float w = maxX - minX;
    float h = maxY - minY;
    ...
    // Background circle
    uint32_t bgColor = isReady() ? VIZ_COL32(30, 30, 80, 255) : VIZ_COL32(60, 30, 30, 255);
    dl->AddCircleFilled(VizVec2(cx, cy), r, bgColor);
    dl->AddCircle(VizVec2(cx, cy), r, VIZ_COL32(100, 100, 100, 255), 32, 2.0f);

    // TX indicator
    uint32_t textColor = isReady() ? VIZ_COL32(100, 150, 255, 255) : VIZ_COL32(180, 180, 180, 255);
    ...
    if (m_packetsSent > 0) {
        ...
        dl->AddText(VizVec2(cx - countSize.x * 0.5f, maxY - countSize.y - 2), VIZ_COL32(100, 150, 255, 200), countStr);
    }
