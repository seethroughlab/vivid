// vivid-audio Example Smoke Tests
// Verifies all vivid-audio examples run without crashing using snapshot mode.
...
static fs::path getTempOutputPath(const std::string& testName) {
    ...
    return tempDir / ("vivid_audio_example_" + testName + ".png");
}
...
TEST_CASE("vivid-audio examples run without crash", "[smoke][examples][audio]") {
    ...
    DYNAMIC_SECTION("Example: " << example) {
        ...
        fs::path examplePath = fs::path(getSourceDir()) / "modules" / "vivid-audio" / "examples" / example;
        ...
        int result = runSnapshot(examplePath.string(), outputPath, 15);
