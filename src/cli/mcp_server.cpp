...
private:
    ...
    json handleToolsList() {
        ...
        // analyze_color_harmony - Extract palette and score color harmony
        tools.push_back({
            {"name", "analyze_color_harmony"},
            {"description", "Extract a 5-color palette from an image and score its color harmony against standard models "
                            "(complementary, analogous, triadic, split-complementary). Returns palette as hex colors, "
                            "best-matching harmony model, and harmony score. Works on any PNG — use capture_frame or "
                            "capture_snapshot first to get an image."},
            {"inputSchema", {
                {"type", "object"},
                {"properties", {
        ...
        // analyze_symmetry - Measure horizontal, vertical, radial symmetry
        tools.push_back({
            {"name", "analyze_symmetry"},
            {"description", "Measure horizontal, vertical, and radial (4-fold) symmetry of an image. Returns scores "
                            "from 0 (asymmetric) to 1 (perfect symmetry) for each axis. Useful for verifying kaleidoscope "
                            "effects, mandala generators, and centered compositions."},
            {"inputSchema", {
                {"type", "object"},
                {"properties", {
                    {"image_path", {{"type", "string"}, {"description", "Path to PNG image to analyze"}}}
        ...
        // analyze_spatial_balance - Rule-of-thirds, bias, quadrant balance
        tools.push_back({
            {"name", "analyze_spatial_balance"},
            {"description", "Analyze compositional balance: rule-of-thirds scoring, horizontal/vertical bias, and quadrant "
                            "weight distribution. Returns thirdsScore (content at power points), bias values (-1 to +1), "
                            "and overall balanceScore (0-1). Works on any PNG."},
            {"inputSchema", {
                {"type", "object"},
                {"properties", {
                    {"image_path", {{"type", "string"}, {"description", "Path to PNG image to analyze"}}}
    ...
    json handleToolsCall(const json& params) {
        ...
        if (name == "get_pending_changes") {
            ...
            if (imagePath.empty()) {
                result["isError"] = true;
                result["content"] = {{{"type", "text"}, {"text", "Required: image_path"}}};
                return result;
            }

            auto img = vivid::io::loadImage(imagePath);
            if (!img.valid()) {
                json response;
                response["success"] = false;
                response["error"] = "Failed to load image: " + imagePath;
                result["content"] = {{{"type", "text"}, {"text", response.dump(2)}}};
                return result;
            }
