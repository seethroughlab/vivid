...
bool HotReload::compile() {
    ...
    // Search upwards to find the vivid root (contains src/vivid-core/include/vivid)
    fs::path rootDir;
    ...
    for (int i = 2; i <= 4; ++i) {
        ...
        fs::path candidateInclude = candidate / "src" / "vivid-core" / "include";
    ...
    if (isDevelopmentMode) {
        ...
        for (const auto& lib : libraries) {
            if (fs::exists(lib.includePath)) {
                depIncludes.push_back(lib.includePath);
            }
        }
    ...
    for (const auto& inc : userLibIncludes) {
        clCmd << "/I\"" << inc.string() << "\" ";
    }
