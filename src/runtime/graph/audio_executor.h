...
public:
    ...
    // Count of recording-tap chunks dropped because the ring filled (drain too
    // slow). Non-zero means the recorded buffer is missing samples. (audit 05-F6)
    uint32_t recording_overrun_count() const { return recording_overrun_count_; }
